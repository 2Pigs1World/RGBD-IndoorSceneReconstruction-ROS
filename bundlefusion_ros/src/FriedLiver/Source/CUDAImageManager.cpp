...
{
       // std::cout << "I'm in CUDAImageImageManager" << std::endl;
	if (!m_RGBDSensor->processDepth()) return false;	// Order is important!
        //std::cout << "proecessDepth" << std::endl;
	if (!m_RGBDSensor->processColor()) return false;
       // std::cout << "proecessColor" << std::endl;
	...
	if (GlobalAppState::get().s_bUseCameraCalibration)
	{
		...
		//m_imageCalibrator.process(DXUTGetD3D11DeviceContext(), d_depthInputRaw, m_SIFTdepthIntrinsics, m_RGBDSensor->getDepthIntrinsicsInv(), m_RGBDSensor->getDepthExtrinsics()); change by guan
...
bool CUDAImageManager::myProcess(cv_bridge::CvImageConstPtr *pColorImage, cv_bridge::CvImageConstPtr *pDepthImage)
{
		std::cout<<"############myProcess"<<std::endl;
       // std::cout << "I'm in CUDAImageImageManager" << std::endl;
	if (!((ImageDataReader *)m_RGBDSensor)->myProcessDepth(pColorImage, pDepthImage)) return false;	// Order is important!
        //std::cout << "proecessDepth" << std::endl;
	if (!m_RGBDSensor->processColor()) return false;
       // std::cout << "proecessColor" << std::endl;
	if (m_currFrame + 1 > GlobalBundlingState::get().s_maxNumImages * GlobalBundlingState::get().s_submapSize) {
