...
CameraInfo kinectPoseFromEigen1(std::pair<Eigen::Matrix3d,Eigen::Vector3d> pos,float fx, float fy, float cx, float cy){
	CameraInfo result;
...
void addMapCallback(const topic_demo::pose::ConstPtr &msg, 
float imageDepthScale, 
float maxCamDistance,
FusionMipMapCPU *fusion,
volatile bool *newMesh)
{
	...
	float q3 = msg->q3;
	float q4 = msg->q4;
	float translation1 = msg->translation1;
	float translation2 = msg->translation2;
	float translation3 = msg->translation3;
			// std::cout<<time<<q1<<q2<<q3<<q4<<translation1<<translation2<<translation3<<std::endl;
	std::cout<<"\n!!!!!!!"<<std::endl;
	...
	std::string depthname = "/home/orbbec/Desktop/dataset/office1/frame-"+std::string(temp)+".depth.png";
	std::string rgbname = "/home/orbbec/Desktop/dataset/office1/frame-"+std::string(temp)+".color.jpg";
				std::cout<<"name"<<std::endl;

	std::pair<Eigen::Matrix3d,Eigen::Vector3d>  pose_from_assfile(Eigen::Quaterniond(q4,q1,q2,q3).toRotationMatrix(),Eigen::Vector3d(translation1,translation2,translation3));
			std::cout<<"pose_from_assfile"<<std::endl;

	CameraInfo pose(kinectPoseFromEigen1(pose_from_assfile,fx,fy,cx,cy));
		std::cout<<depthname<<std::endl;
		std::cout<<rgbname<<std::endl;

	//cv::Mat depthimage = cv::imread(depthname,-1);
	//cv::Mat rgbimage = cv::imread(rgbname);
	// cv_bridge::CvImageConstPtr pColorImage = cv::toCvCopy(msg->colorImage, msg->colorImage->encodeing);
	// cv_bridge::CvImageConstPtr pDepthImage = cv::toCvCopy(msg->depthImage, msg->depthImage->encodeing);
	cv::Mat rgbimage(cv_bridge::toCvCopy(msg->colorImage, msg->colorImage.encoding)->image);
	cv::Mat depthimage(cv_bridge::toCvCopy(msg->depthImage, msg->depthImage.encoding)->image);
	// std::cout<< rgbimage<<std::endl;
	// cv::imwrite("/home/orbbec/Desktop/tmp.png", rgbimage);
	
	std::cout<< "\n#######read!\n"<<std::endl;

	fusion->addMap(depthimage,pose,rgbimage,1.0f/imageDepthScale,maxCamDistance);
	std::cout<< "\n#######add!\n"<<std::endl;
	*newMesh = fusion->updateMeshes();
		std::cout<< "\n#######fused!\n"<<std::endl;
...
)
{
  ...
  unsigned int startFrame = *_currentFrame+1;
	...
	ros::Subscriber sub = n.subscribe<topic_demo::pose>("pose_info", 1, boost::bind(&addMapCallback, _1, imageDepthScale, maxCamDistance, fusion, newMesh )); 
...
void OnlineFusionViewerManipulated::updateSlot()
{
	if(_threadFusion){
		...
		if(!_fusionThread){
			_fusionThread = new boost::thread(myFusionWrapper,_depthNames,_rgbNames,_poses,
